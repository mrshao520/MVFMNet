...
def calculate_sam(img, img2, crop_border, input_order='HWC', test_y_channel=False, **kwargs):
    """Calculate SAM (Spectral Angle Mapper). 光谱角制图
...
@METRIC_REGISTRY.register()
def calculate_uqi(img, img2, crop_border, input_order='HWC', test_y_channel=False, **kwargs):
    """Calculate UQI (Universal Image Quality Index). 通用图像质量指数
    UQI值越大，图像质量越高。
...
@METRIC_REGISTRY.register()
def calculate_scc(img, img2, crop_border, input_order='HWC', test_y_channel=False, **kwargs):
    """Calculate SCC (Spatial Correlation Coefficient). 空间相关系数
    SCC值越大，图像质量越高。
